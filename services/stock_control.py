...
def create_daily_news_url(url4, title4, intro4, cover4):
    with Session(engine) as session:
        dailynews = DailyNewsUrl(url=url4, title=title4, intro=intro4, cover=cover4)
...
def modify_company_basic_info(keyword, condition, Target):
    with Session() as session:
        basic = (
            Session.query(company_basic_info)
            .filter_by(company_basic_info.keyword == condition)
            .first()
        )
...
def modify_company_financial_info(keyword, contidion, Target):
    with Session() as session:
        financial = (
            Session.query(company_financial_info)
            .filter_by(company_financial_info.keyword == contidion)
            .first()
        )
...
def modify_company_shaerholder_info(keyword, condition, Target):
    with Session() as session:
        shareholder = (
            Session.query(company_shareholder_info)
            .filter_by(company_shareholder_info.keyword == condition)
            .first()
        )
...
def modify_daily_news_url(keyword, condition, Target):
    with Session() as session:
        dailynews = (
            Session.query(daily_news_url)
            .filter_by(daily_news_url.keyword == condition)
            .first()
        )
...
def modify_kline_graph_info(keyword, condition, Target):
    with Session() as session:
        kline = (
            Session.query(kline_graph)
            .filter_by(kline_graph_info.keyword == condition)
            .first()
        )
...
def modify_transaction_data_seperate(keyword, condition, Target):
    with Session() as session:
        transaction = (
            Session.query(transaction_data_seperate)
            .filter_by(transaction_data_seperate.keyword == condition)
            .first()
        )
...
def delete_company_basic_info(keyword, condition):
    with Session() as session:
        basic = (
            Session.query(company_basic_info)
            .filter_by(compamy_basic_info.keyword == condition)
            .first()
        )
...
def delete_company_financial_info(keyword, condition):
    with Session() as session:
        financial = (
            Session.query(company_financial_info)
            .filter_by(company_fiancial_info.keyword == condition)
            .first()
        )
...
def delete_kline_graph_info(keyword, condition):
    with Session() as session:
        kline = (
            Session.query(kline_graph)
            .filter_by(kline_graph.keyword == condition)
            .first()
        )
...
def company_basic_info_query(keyword, condition):
    with Session() as session:
        basic = (
            Session.query(company_basic_info)
            .filter(company_basic_info.keyword == condition)
            .all()
        )
...
def company_financial_info_query(keyword, condition):
    with Session() as session:
        financial = (
            Session.query(company_financial_info)
            .filter(company_financial_info.keyword == condition)
            .all()
        )
...
def company_shareholder_info_query(keyword, condition):
    with Session() as session:
        shareholder = (
            Session.query(company_shareholder_info)
            .filter(company_shareholder_info.keyword == condition)
            .all()
        )
...
def daily_news_url_query(keyword, condition):
    with Session() as session:
        dailynews = (
            Session.query(daily_news_url)
            .filter(daily_news_url.keyword == condition)
            .all()
        )
...
def kline_graph_query(keyword, condition):
    with Session() as session:
        kline = (
            Session.query(kline_graph).filter(kline_graph.keyword == condition).all()
        )
...
def transaction_data_seperate_query(keyword, condition):
    with Session() as session:
        transaction = (
            Session.query(transaction_data_seperate)
            .filter(transaction_data_seperate.keyword == condition)
            .all()
        )
