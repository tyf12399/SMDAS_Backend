...
def update_discretionary_stock(keyword, condition, Target):
    with Session() as session:
        discretionary = Session.query(DiscretionaryStock).filter_by(keyword == condition).first()
...
def update_personality_module(keyword, condition, Target):
    with Session() as session:
        personality = Session.query(PersonalityModule).filter_by(keyword == condition).first()
...
def discretionary_stock_query(account, stock_code):
    with Session() as session:
        discretionary = select(DiscretionaryStock).where(DiscretionaryStock.account.in_([account]))\
            .where(DiscretionaryStock.stock_code.in_([stock_code]))


def personality_module_query(account):
    with Session() as session:
       personality = select(PersonalityModule).where(PersonalityModule.account.in_([account]))