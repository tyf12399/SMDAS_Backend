...
def create_discretionary_stock(account1, stockcode1):
    with Session() as session:
        discretionary = DiscretionaryStock(account=account1, stockcode=stockcode1)
...
def record_modify(self):
    def modify_discretionary_stock(self, keyword, condition, Target):
        with Session() as session:
            discretionary = (
                Session.query(discretionary_stock)
                .filter_by(keyword == condition)
                .first()
            )
...
def record_delete(self):
    def delete_discretionary_stock(self, keyword, condition):
        with Session() as session:
            discretionary = (
                Session.query(discretionary_stock)
                .filter_by(discretionary_stock.keyword == condition)
                .first()
            )
...
def info_query(self):
    def discretionary_stock_query(self, keyword, condition):
        with Session() as session:
            discretionary = (
                Session.query(discretionary_stock)
                .filter(discretionary_stock.keyword == condition)
                .all()
            )
            data = [
                dict(zip(discretionary.keys(), discretionary))
                for results in discretionary
            ]
    ...
    def personality_module_query(self, keyword, condition):
        with Session() as session:
            personality = (
                Session.query(personality_module)
                .filter(personality_module.keyword == condition)
                .all()
            )
            data = [
                dict(zip(personality.keys(), personality)) for result in personality
            ]
