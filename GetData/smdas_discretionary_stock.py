...
def modify_discretionary_stock(self):
    Discretionary = Session.query(discretionary_stock).filter_by(Query Criteria).first()
    Discretionary.attribute = 'Target'
    Session.commit()

def modify_personality_module(self):
    Personality = Session.query(personality_module).filter_by(Query Criteria).first()
...
def discretionary_stock_query(self):
    Discretionary = Session.query(discretionary_stock).filter(keyword condition).all()

def personality_module_query(self):
    Personality = Session.query(personality_module).filter(keyword condition).all()